...
function VideoCard(props) {
    ...
    return (
        ...
        <div className=  {`flex-row card rounded-none w-full h-full cursor-pointer m-auto`}>
            <Link to={`/watch/${props.id}`} className='group flex-1 relative aspect-[16/9] text-xs bg-base-100 object-contain rounded-xl overflow-hidden'>
            ...
            <div className='flex flex-col w-[40%] md:w-4/6 mx-3 md:mx-5'>
                <Link to={`/watch/${props.id}`} className='tilteTruncate text-sm sm:text-base xl:text-xl font-medium'>
                  {props.videoTitle}
                </Link>

                <div className='flex flex-col font-normal text-xs sm:text-xs xl:text-xs '>

                    <span className='opacity-80'> {getYoutubeViews(props.viewsNum)} views • {getYoutubeRelativeTime(props.period)} ago</span>
                    
                    <div className="flex flex-row items-center my-2 mx-2 sm:mx-0 ">
                        
                        <Link to={`/channel/${props.channelID}`} className=' mr-2 avatar'>
                            <div className="w-6 h-6 rounded-full m-auto">
                    ...
                    <span className='tilteTruncate opacity-80'> {props.description} </span>
                </div>
